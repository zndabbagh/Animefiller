...
// -------------------------
const caseClosedSeasons = [
    28, 26, 28, 24, 28, 28, 31, 26, 35, 31, 30, 38, 36, 37, 39,
    25, 33, 42, 40, 40, 35, 43, 39, 41, 83, 40, 38, 28, 40, 35,
    41, 39, 33
];

const caseClosedMapping = {};
let cumulative = 1;
for (let i = 0; i < caseClosedSeasons.length; i++) {
    caseClosedMapping[i + 1] = cumulative;
    cumulative += caseClosedSeasons[i];
}

// -------------------------
// One Piece season mapping
// -------------------------
const onePieceStartEpisodes = {
...
function getOnePieceAbsoluteEpisode(season, episode) {
    if (!onePieceStartEpisodes[season]) return null;
    return onePieceStartEpisodes[season] + (episode - 1);
}

function getAbsoluteEpisode(animeName, season, episodeInSeason) {
    if (animeName === 'case-closed' || animeName === 'detective-conan') {
        return caseClosedMapping[season] + episodeInSeason - 1;
    } else if (animeName === 'one-piece') {
        return getOnePieceAbsoluteEpisode(season, episodeInSeason);
    } else {
